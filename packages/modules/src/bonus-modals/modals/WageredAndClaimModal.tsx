...
import { buyBmBonus, claimBmBonus } from '@oribet/core/api/services/BmBonus.api'
...
function WageredAndClaimModal({
  ...
  const activeBmBonus = useFetchActiveBmBonus(true)

  const moveToMainBalanceAndBuyBonus = () => {
    setLoading(true)
        ...
        if (res.success) {
          ...
          buyBmBonus(params)
            .then(res => {
              ...
              Defaults.modals.wageredAndClaimModal?.hide()
            })
            .catch(err => {
              Defaults.modals.wageredAndClaimModal?.hide()
  ...
  return (
    <div>
      ...
      <Paragraph>
        Wagering Done. <br></br>Your previous bonus is ready to be transferred. Move it to your real
        balance and claim the next one now.
      </Paragraph>
      <ButtonsWrapper>
        <Button>
          <Label>Bonus Money</Label>
          <Price>{getActiveCurrencySymbol()}{formatAmount(Number(data.amountClaimed))}</Price>
        </Button>
        <Button>
      ...
      <ButtonsWrapper style={{ paddingTop: '0px' }}>
        <Button>
          ...
          <Price>{getActiveCurrencySymbol()}{formatAmount(Number(activeBmBonus?.data?.data?.amount || ''))}</Price>
        </Button>
      </ButtonsWrapper>

      <ButtonContainer>
        <ActionButton
          data-testid={`${BONUSES_TEST_IDS.modal.action}.wagered-and-claim.confirm`}
          color="orange"
          buttonType=""
          withIcon={false}
          disabled={loading}
          onClick={moveToMainBalanceAndBuyBonus}
          label="Transfer & Claim New Bonus"
          style={{ height: '32px' }}
        />
      </ButtonContainer>
    </div>
...
export default WageredAndClaimModal
